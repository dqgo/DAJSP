...
function [nei_chromos]=create_nei(schedule,key_index,keyblock_schedule,chromo,datatemp)
    nei_chromos=[];
    nei_schedule=[];
    schedule(:,10)=-1;
        ...
        if this_keyblock_index(1)<size(chromo{2},2)
            if size(this_keyblock,1)==2
                ...
                if isOk
                    ...
                    nei_schedule=cat(nei_schedule,this_nei_schedule,3);
                end
            else
                ...
                for point =2:this_block_size-1
                    ...
                    this_keyblock(1,10)=0;this_keyblock(2:point-1,10)=1;this_keyblock(point,10)=2;
                    ...
                    this_keyblock(point,10)=0;this_keyblock(point+1:end-1,10)=1;this_keyblock(end,10)=2;
                    ...
                    if can_U2Vnext(u_schedule,v_schedule,tail_data)
                        ...
                        for re=point:this_block_size
                            ...
                            this_move_block(re,1:3)=this_move_block_before_arrange(re,1:3);
                ...
                % isOk=can_V2Uper(u_schedule,v_schedule,schedule);
                % if isOk
                %     this_nei_schedule_withno_assembly(this_keyblock_index(1:2),:)=V2Uper(u_schedule,v_schedule);
                %     this_nei_chromo{2}=this_nei_schedule_withno_assembly(:,1)';
                %     nei_chromos=[nei_chromos;this_nei_chromo];
                % end
                % 
                ...
                % isOk=can_V2Uper(u_schedule,v_schedule,schedule);
                % if isOk
                %     this_nei_schedule_withno_assembly(this_keyblock_index(end-1:end),:)=V2Uper(u_schedule,v_schedule);
                %     this_nei_chromo{2}=this_nei_schedule_withno_assembly(:,1)';
                %     nei_chromos=[nei_chromos;this_nei_chromo];
                % end
            end
